...
function window_OnError(message, url, linenumber) {
    kesco_OnError(1, message, url, linenumber);
    return true;
}

function kesco_OnError(type, message, url, linenumber) {
    if (window.XMLHttpRequest && 1==0) {
        var xhr = new XMLHttpRequest();
        var scripturl = "?_rc=error";

        var params = "type=" + type + "&linenumber=" + linenumber + "&message=" + encodeURIComponent(message) + "&errorUrl=" + encodeURIComponent(url);

...
var v4_isSignal = true;
/*Ссылка с параметрами на хендлер Signal*/
var v4SignalUrl_Template = "";
...
/*Глобальная переменная, задающая дива загрузки progressBar*/
var v4_pageLoad_Progess_IdPart = "v4_pageLoad_Progess";
...
var page_clientLocalization = null;

function window_OnError(message, url, linenumber) {
    v4_OnError(1, message, url, linenumber);
    return true;
}

function v4_OnError(type, message, url, linenumber) {
    ...
    $.post(
        "DialogResult.ashx",
        {
            ...
            control: "window",
            command: "error",
            messageError: encodeURIComponent("[V4_ScriptError]:"+message),
            typeError: type,
            linenumberError: linenumber,
            urlError: encodeURIComponent(url),
        }
    );
}
...
var v4_buttonIcons = {
    ...
    VolumeOff: "ui-icon-volume-off",
    Download: "ui-icon-arrowstop-1-s",
    Reload: "ui-icon-arrowrefresh-1-e",
	BackToList: "icon-backtolist",
	InDoc: "ui-icon-indoc",
	Call: "icon-call"
}
...
function v4_keyDown(event) {
    ...
    if (v4_helpURL != null && key == 112) {
        ...
        v4s_hidePopup(true);
    } else if (!event.ctrlKey && !event.altKey && !event.shiftKey && key == v4_keys.insert) {
        window.v4_insert();
    }
...
function v4_replaceStyleRequired(o) {
    if (o == null || o.id==null || o.id=="") return;
    if ((o.value == null || o.value.trim() == '') && o.getAttribute('isRequired') == 1) {
        $("#" + o.id).addClass("v4s_required");
    } else {
        $("#" + o.id).removeClass("v4s_required");
    }
}
...
function v4_openHelp(idpage) {
    ...
    Kesco.windowOpen(v4_helpURL + url_params, currentPage == null ? '_blank' : currentPage);
}
...
/*Функция удаления из KescoHub информации об странице с последующим закрытием ее в браузере*/
...
function v4_closeWindowByIdp(widp, isframe) {
    ...
    var url = "DialogResult.ashx?control=window&command=pageclose&callbackKey=" + widp + frameParams;
    $.ajax({
        async: false,
        type: "POST",
        url: url
...
function v4_SrvSendWindowSizePos() {
    window.removeEventListener('beforeunload', v4_SrvSendWindowSizePos, false);

    var isDialog = (window.dialogWidth && window.dialogHeight);
    var cw = 0;
    var ch = 0;
    if (isDialog) {
...
function v4_setSizeFormContainer() {
    var formHeight = 0;
    var h = 0;
    var offHeight = $(window).height();
   // console.log("window -> offHeight:" + offHeight);

    h = $(".v4pageHeader").outerHeight(true);
    offHeight -= h; formHeight += h;
    //console.log("v4pageHeader -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight );
    
    $(".v4pageDocHeader").each(function() {

        h = $(this).outerHeight(true);
        offHeight -= h; formHeight += h;
     //   console.log("v4pageDocHeader -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight);
    });
    
    h = $(".v4pageTitle").outerHeight(true);
    offHeight -= h; formHeight += h;
    //console.log("v4pageTitle -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight);

    $(".v4divHeader").each(function () {
        h = $(this).outerHeight(true);
        offHeight -= h; formHeight += h;
      //  console.log("v4divHeader -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight);
    });

    h = $(".v4formFilter").outerHeight(true);
    offHeight -= h; formHeight += h;
    //console.log("v4formFilter -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight);

    h = $(".v4pageFooter").outerHeight(true)
    offHeight -= h; formHeight += h;
    //console.log("v4pageFooter -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight);

    h = offHeight - ((opener || parent) ? 10 : 25)
    //console.log("v4formContainer -> " + h + "-> offHeight:" + offHeight + "-> formHeight:" + formHeight);
    var mh = $(".v4formContainer").css('min-height');
    if (h < mh) {
        $(".v4formContainer").height(mh);
        formHeight += mh;
    } else {
        $(".v4formContainer").height(h);
        formHeight += h;
    }
    $("#v4form").height(formHeight);

    //var wh = $(window).height();
   // console.log("v4form -> " + "-> formHeight:" + formHeight + "-> wh:" + wh);
}



function v4_containerAppendFragmentByClass(container, className) {
...
function v4_init() {
    ...
    $('body').append('<div id=\"v4form\"></div>');
    var v4form = document.getElementById('v4form');

    v4_containerAppendFragmentByClass(v4form, "v4pageHeader");
    v4_containerAppendFragmentByClass(v4form, "v4pageDocHeader");
    v4_containerAppendFragmentByClass(v4form, "v4pageTitle");
    v4_containerAppendFragmentByClass(v4form, "v4divHeader");
    v4_containerAppendFragmentByClass(v4form, "v4formFilter");
    v4_containerAppendFragmentByClass(v4form, "v4formContainer");
    v4_containerAppendFragmentByClass(v4form, "v4pageFooter");
    
    v4_setSizeFormContainer();
    ...
    $(window).unload(function () {
        ...
        if (window.self !== window.top) {
            ...
            if (typeof idp === 'undefined' || idp === null) return;
...
/*Функция установки активного элемента документа при получении им форкуса
event - текущее событие
*/
...
function v4s_init() {
    ...
    var $div_p = $('<div />').appendTo('body');
    if (!$div_p) return;

    $div_p.attr('id', 'v4s_popup');
    $div_p.attr('ids', '');

    $div_p.css("border-left", borderStyle);
    $div_p.css("border-right", borderStyle);
    $div_p.css("border-bottom", borderStyle);
    $div_p.css("position", "absolute");
    $div_p.css("max-height", "250px");
    $div_p.css("overflow", "auto");
    $div_p.css("display", "none");
    $div_p.css("z-index", "21000");

    $div_p.bind("mouseover", v4s_popupOver);
    $div_p.bind("mouseout", v4s_popupOut);
    $div_p.bind("mousedown", v4s_mouseDown);
    $div_p.bind("click", v4s_popupClick);

    v4s_popup = document.getElementById("v4s_popup");
}
...
function v4s_popupClick(event, rowIndex) {
    ...
    if (o.getAttribute('cmd') == 'select') {
        ...
        cmd('ctrl', id, 'vn', vn, 'tn', text, 'next', 1);
...
function v4s_setSelectedValue(control, callbackKey, result, isMultiReturn) {
    ...
    if (typeof result === "object") {
        for (var i = 0; i < result.length; i++) {
            if (result[i].isAddress != null && result[i].isAddress == true)
                result[i].value = '1'+result[i].value;
...
*/
function v4s_btnStyle(id, windowParams) {
    var idInp = id + '_0';
	var inp = gi(idInp);
    var btn = gi(id + '_1');
    if (btn == null) return;
    ...
    if (inp.value != '' && inp.value == inp.getAttribute('t') && (btn.getAttribute('urlShowEntity') != null || btn.getAttribute('funcShowEntity') != null)) {
		...
		if ($( "#" + idInp).hasClass( "v4_callerControl" ))
		{			
			var dataId = $( "#" + idInp)[0].getAttribute('data-id')
			if (dataId == null || dataId == '') return;
			$( "#" + idInp).initToolTip(v4_tooltipCaller, $(document.body));			
		}
		
        if (btn.getAttribute('urlShowEntity') != null)
            btn.onclick = function () { 
					var _urlShowEntity = btn.getAttribute('urlShowEntity');
                Kesco.windowOpen(_urlShowEntity + (_urlShowEntity.indexOf("?") > 0 ? "&" : "?") + 'id=' + inp.getAttribute('v'), '', windowParams, btn); 
			};
...
function v4s_btnStyle4Popup(btn, id) {
	...
	var idInp = id + '_0';
	if ($( "#" + idInp).hasClass( "v4_callerControl" ))
		$( "#" + idInp).qtip('destroy', true);
	
	if ( !$( "#" + idInp ).prop( "v" ) || $( "#" + idInp ).prop( "v" )=="")
		$( "#" + idInp).prop('title', '');
...
var v4s_onBlur = function (event) {
    ...
    var nolimit = o.getAttribute('nolimit') == "true";

    if (!v4s_isPopupOpen && !nolimit) {
        ...
        if (document.activeElement == null || document.activeElement.id !== id) {
            if (o.value != o.getAttribute('t')) {
                if (nolimit) {
                    cmdasync('ctrl', id, 'vn', o.value, 'tn', o.value, 'next', 1);
                } else {
                    o.value = '';
                    cmdasync('ctrl', id, 'tn', o.value);
                }
...
function v4s_textChange(event, id, x) {
    ...
    if (o.value != null && (o.value.length == 0 || o.value != o.getAttribute('t'))) {
        ...
        if (btn != null && (btn.getAttribute('urlShowEntity') != null || btn.getAttribute('funcShowEntity') != null)) {
            ...
            btn.className = 'v4s_btnDetail';
            if (btn.getAttribute('urlShowEntity') != null)
                btn.onclick = function () { Kesco.windowOpen(btn.getAttribute('urlShowEntity') + '?id=' + o.getAttribute('v'), '', btn); };
            if (btn.getAttribute('funcShowEntity') != null)
                btn.onclick = function () {
                    var _scr = $.validator.format(btn.getAttribute('funcShowEntity'), inp.getAttribute('v'), id);
                    eval(_scr);
    ...
    if (v4s_isPopupOpen && e.keyCode != 13 && e.keyCode != 27) {
        ...
        if (btn != null) {
            if ((btn.getAttribute('urlShowEntity') != null || btn.getAttribute('funcShowEntity') != null) &&
                ...
                if (btn.getAttribute('urlShowEntity') != null)
                    btn.onclick = function() {
                        Kesco.windowOpen(btn.getAttribute('urlShowEntity') + '?id=' + o.getAttribute('v'), '', btn);
                    };
                if (btn.getAttribute('funcShowEntity') != null)
                    btn.onclick = function () {
                        var _scr = $.validator.format(btn.getAttribute('funcShowEntity'), inp.getAttribute('v'), id);
...
function v4t_keyDown(event) {
    ...
    if (e.keyCode == 13 && o.nodeName != 'TEXTAREA') {
        v4_setFocus2NextCtrl();
        return false;
    }

    /*чтобы уходил с последнего элемента*/
    if ((o.nodeName == 'TEXTAREA' || o.nodeName == 'INPUT') && e.keyCode == 9) {
...
event-текущее событие
*/
function v4t_keyUp(event) {
    event = window.event || event;
    var e = event;
    var o = e.target || e.srcElement;
    if (e.keyCode == 27) return;
    if (o.getAttribute('nm') != 'True') {
        if (o.getAttribute('ov') != o.value) {
            v4_SetSaveButtonDisabled(false);
        } else {
            var isChange = v4t_CheckChange();
            v4_SetSaveButtonDisabled(!isChange);
        }
    }
    v4_replaceStyleRequired(o);
}

/*Функция возвращает true, если были изменения контролов на странице
*/
function v4t_CheckChange(id) {
    var isChange = false;
    $('body').find('input[type=text], input[type=checkbox], textarea').each(function () {
        if (!$(this).attr('nm') || $(this).attr('nm') != 'True') {
            if (id == null || id != ($(this)).attr('id')) {
                if ($(this).attr('type') == 'checkbox') {
                    if ($(this).prop("checked") == true && $(this).attr('ov') != '1') {
                        isChange = true;
                    }
                    if ($(this).prop("checked") == false && $(this).attr('ov') != '0') {
                        isChange = true;
                    }
                }
                else if (!$(this).attr('v')) {
...
function v4_SetSaveButtonDisabled(status) {
	setTimeout(function(){
		if (status == true) {
			$('#btnApply').prop('disabled', true).addClass('ui-state-disabled').removeClass('ui-state-default');
			$('#btnSave').prop('disabled', true).addClass('ui-state-disabled').removeClass('ui-state-default');
			$('#btnReCheck').prop('disabled', true).addClass('ui-state-disabled').removeClass('ui-state-default');
		} else {
			$('#btnApply').prop('disabled', false).removeClass('ui-state-disabled').addClass('ui-state-default');
			$('#btnSave').prop('disabled', false).removeClass('ui-state-disabled').addClass('ui-state-default');
			$('#btnReCheck').prop('disabled', false).removeClass('ui-state-disabled').addClass('ui-state-default');		
		}
	},10);
}
...
function v4_ctrlChanged(id, goToNextControl, isDatePicker, isMonthFormat) {
    ...
    else
    {
        ...
        if (o.value != o.getAttribute('t')) {
            ...
            if (goToNextControl && $('#' + o.id).is(':focus')) {
                v4_setFocus2NextCtrl(id + '_0');
            }
        } else if (goToNextControl) {
            v4_setFocus2NextCtrl(id + '_0');
...
$(document).ready(function () {
    ...
    $(document).find('input, textarea, select').each(function () {
        if ($(this).attr('nv') == '1') {
            var ov = $(this).attr('ov');
            var t = $(this).attr('t');
            $(this).removeClass('v4_modified');
            if ($(this).attr('ov') !== undefined) {
                if ($(this).attr('type') == 'checkbox') {
                    if ($(this).attr('ov') == 1 && $(this).prop('checked') == false ||
                        $(this).attr('ov') != 1 && $(this).prop('checked') == true) {
                        $(this).addClass('v4_modified');
                    }
                } else {
                    if ($(this).attr('v') !== undefined) {
                        if ($(this).attr('ov') != $(this).attr('v'))
                            $(this).addClass('v4_modified');
                    } else {
                        if ($(this).hasClass("v4n")) {
                            var ov_f = parseFloat(ov);
                            var t_f = parseFloat(t);
                            if (!(isNaN(ov_f) && t_f == 0 || isNaN(t_f) && ov_f == 0)) {
                                if (ov_f != t_f) {
                                    $(this).addClass('v4_modified');
                                }
                            }
                        } else {
                            if ($(this).attr('ov') != $(this).attr('t')) {
                                $(this).addClass('v4_modified');
                            }
                        }
                    }
    ...
    /*скрытие дива  загрузки после загрузки*/
    if (parent != null) {
        var dialogId = v4_getQSParamValue('dialogId');
        if (dialogId != undefined) {
            var pbar = parent.$("#" + v4_pageLoad_Progess_IdPart + '_' + dialogId);
            if (pbar != null) pbar.hide();
        }
    }
});
...
function v4_polymorph() {
    ...
    return function () {
        if (len2func[arguments.length] == undefined) {
...
function v4_setDialogFrameSize(idFrame, idContainer) {
    if (idFrame == null) {
        var parts = idContainer.split('_')
        //idFrame = "ifrEntity_" + parts[1] + "_" + parts[2];
        idFrame = "ifrEntity_" + parts[1];
    }

    $("#" + idFrame).height($("#" + idContainer).height());
    $("#" + idFrame).width($("#" + idContainer).width());
}
...
var v4_dialog = v4_polymorph(
    ...
    function (id, htmlContent, dialogDiv, title, status, width, height, onOpen, onClose, buttons, dialogPosition, isForm, titleCloseButton, dialogConfirm, modalView) {
        ...
        if (!$("div").is("#OpenMultiWin")) {
            var multiWinDiv = document.createElement('div');
            multiWinDiv.setAttribute('id', 'OpenMultiWin');
            document.body.appendChild(multiWinDiv);
        }

        var multiWinContainer = document.getElementById("OpenMultiWin");

        if (dialogDiv != null) {
            var fragment = document.createDocumentFragment();
            var ids = document.getElementsByClassName("EntityAddContainer");
            var inx = 0;
            for (var i = 0; i < ids.length; i++) {
                inx++;
                fragment.appendChild(ids[i]);
            ...
            if (inx > 0)
                multiWinContainer.appendChild(fragment);
        }

        if (dialogDiv != null)
            dialogObj = dialogDiv; //TODO: удалить в будущем
        else {
            dialogObj = $(document.createElement('div')).attr("id", id);
            //dialogObj = $(multiWinContainer.createElement('div')).attr("id", id);
        }
        ...
        dialogObj.dialogExtend({
            "closable": true,
            "maximizable": modalView ? false : true,
            "minimizable": modalView ? false : true,
            "collapsable": modalView ? false : false,
            "maximize": function (e, dlg) { v4_setDialogFrameSize(null, this.id); },
            "restore": function (e, dlg) { v4_setDialogFrameSize(null, this.id); }
        });

        if (page_clientLocalization) {
            $("a[title|='minimize']").attr('title', page_clientLocalization.minimize_button);
            $("a[title|='maximize']").attr('title', page_clientLocalization.maximize_button);
            $("a[title|='collapse']").attr('title', page_clientLocalization.collapse_button);
            $("a[title|='restore']").attr('title', page_clientLocalization.restore_button);
            $("a[role|='button'] span[title|='restore'").attr('title', page_clientLocalization.restore_button);
            $("button[title|='Close']").attr('title', page_clientLocalization.close_button);
        }
...
var v4_showConfirm = v4_polymorph(
    function (message, title, captionYes, captionNo, callbackYes, width) {
        v4_showConfirm(message, title, captionYes, captionNo, 75, 75, callbackYes, "", "", width, null, 1, 0, null, null)
    },

    function (message, title, captionYes, captionNo, callbackYes, callbackNo, width) {
        v4_showConfirm(message, title, captionYes, captionNo, 75, 75, callbackYes, callbackNo, "", width, null, 1, 0, null, null)
    },
	
	function (message, title, captionYes, captionNo, widthYes, widthNo, callbackYes, callbackNo, ctrlFocus, width, height) {
        v4_showConfirm(message, title, captionYes, captionNo, widthYes, widthNo, callbackYes, callbackNo, ctrlFocus, width, height, 1, 0, null, null)
    },

    function (message, title, captionYes, captionNo, widthYes, widthNo, callbackYes, callbackNo, ctrlFocus, width, height, inxBtnFocus, showBtnImg, captionCancel, callbackCancel) {
        ...
        var buttons = [
        {
            ...
            text: captionYes,
            icons: {
                primary: showBtnImg==1 ? v4_buttonIcons.Ok: ""
            },
            width: widthYes,
            click: function () {
                eval(callbackYes);
        ...
        },
         {
             ...
             text: captionNo,
             icons: {
                 primary: showBtnImg == 1 ? v4_buttonIcons.Cancel: ""
             },
             width: widthNo,
             click: function () {
        ...
        if (captionCancel && captionCancel.length>0) {
            var buttonSet = $('#' + dialogId).parent().find('.ui-dialog-buttonset');
            var cancelButton = $('<button>' + captionCancel + '</button>');
            cancelButton.button().click(function () {
                if (null != callbackCancel && "" != callbackCancel) eval(callbackCancel);
                $('#' + dialogId).dialog("close");
            });
...
function v4_showMessage(message, title, status, ctrlFocus, width, height, scriptOk, noButtons) {
...
function v4_createDialogElements() {
    ...
    if (v4_DialogOL == null) {
        $('body').prepend("<div id='v4_divDialogOverlay' style='z-index: 99998'></div>");
...
function v4_setResizableInDialog() {
    if (window.dialogWidth && window.dialogHeight) {
...
function v4_element_keydown(event, element) {
    var key = v4_getKeyCode(event);
    if (key == 13 || key == 32) {
        event.preventDefault();
        element.click();

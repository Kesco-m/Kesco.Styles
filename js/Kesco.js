var Kesco = {
    ...
    globals: {
        ...
        dragDrop: {
            ...
            clear: function () {
                Kesco.globals.dragDrop.dragElement= "";
                Kesco.globals.dragDrop.targetId= "";
                Kesco.globals.dragDrop.targetType= null;
                Kesco.globals.dragDrop.data= "";
                Kesco.globals.dragDrop.containerId= "";
                Kesco.globals.dragDrop.cmdListnerIndex= null;

                Kesco.globals.dragDrop.dropClass= "";
                Kesco.globals.dragDrop.dropIds= null;
                Kesco.globals.dragDrop.dropFunction= null;
            }
...
$(document).ready(function () {
    ...
    //Kesco.event.add(document, 'drag', function (event) {});
    //Kesco.event.add(document, 'dragenter', function (event) {});
    //Kesco.event.add(document, 'dragleave', function (event) {});


    Kesco.event.add(document, 'dragstart', function (event) {
        Kesco.globals.dragged = event.target;
        event.target.style.color = 'black';

    });

    Kesco.event.add(document, 'dragend', function (event) {
        event.target.style.color = "";       
    });

    
    Kesco.event.add(document, 'dragover', function (event) {  
        

        if (Kesco.globals.dragDrop.dropClass != "" && event.target.classList.contains(Kesco.globals.dragDrop.dropClass)
            || Kesco.globals.dragDrop.dropIds != null
            && event.target.id != null
            && event.target.id != ""
            && jQuery.inArray(event.target.id, Kesco.globals.dragDrop.dropIds) > -1) {

            event.preventDefault();
        }
        else if (Kesco.globals.dragDrop.dragElement === "column" && event.target.getAttribute("column-id") != null) {
            event.preventDefault();
        }
    ...
    Kesco.event.add(document, 'drop', function (event) {
        event.preventDefault();

        if (Kesco.globals.dragDrop.dropFunction != null) {

            Kesco.globals.dragDrop.targetId = event.target.getAttribute("column-id");

            if (Kesco.globals.dragDrop.targetId != null && Kesco.globals.dragDrop.targetId != "") {
                Kesco.globals.dragDrop.targetType = 'TH';
            }

            Kesco.globals.dragDrop.dropFunction();           
        }

        Kesco.globals.dragDrop.clear();
    });
